...
class VGGFaceDataset(Dataset):
    def __init__(self, img_dir, file_list_path, transform=None, augmentation=None, device=None):
        ...
        classes = [d for d in listdir(img_dir) if isdir(join(img_dir, d))]
        self.ann = {c: i for i, c in enumerate(set(classes))}

        self.img = []
        for c in classes:
            images = [join(c, f) for f in listdir(join(img_dir, c)) if isfile(join(img_dir, c, f))]
            self.img.extend(images)
    ...
    def __getitem__(self, idx):
        ...
        classname = self.img[idx].split("/")[0]
        ann = torch.tensor(self.ann[classname])
