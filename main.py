...
def start_training(model, dataset, data_sampler, config, classes):
    ...
    model_optimizer = optim.AdamW(model.parameters(), lr=5e-6)
    loss_optimizer = optim.SGD(criterion.parameters(), lr=1e-5)
    load_checkpoint(model, model_optimizer, loss_optimizer, criterion, data_sampler.sampler, config)
    model_scheduler = lr_scheduler.CosineAnnealingLR(model_optimizer, config.num_of_epoch * len(dataset) // 5, 5e-9)
    loss_scheduler = lr_scheduler.CosineAnnealingLR(loss_optimizer, config.num_of_epoch * len(dataset) // 5, 1e-8)
