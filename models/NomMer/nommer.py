...
def gumbel_softmax(logits, device, tau=1, hard=False):
    """
    ST-gumple-softmax
    input: [*, n_class]
...
class HybridAttention(nn.Module):
    def __init__(self, device, dim, heads=8, dropout=0.0, depth=0, wsize=-1, psize=-1, cnn_expansion=4):
    ...
    def forward(self, x):
        ...
        if self.training:
            logits = gumbel_softmax(logits, self.device, tau=1, hard=True)
        else:
...
class HybridNet(nn.Module):
    def __init__(self, device, dim, depth, heads, mlp_dim, wsize, psize, cnn_expansion, dropout=0.0, drop_path=0.0):
        super().__init__()
        self.dim = dim
        self.depth = depth
